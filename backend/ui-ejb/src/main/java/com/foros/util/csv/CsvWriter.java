...
/**
 * A stream based writer for writing delimited text data to a stream.
 */
public class CsvWriter {
    ...
    private boolean appendLineEndingChar = true;

    /**
     * Creates a {@link  com.foros.util.csv.CsvWriter CsvWriter} object using a Writer
     * to write data to.
     *
    ...
    /**
     * Creates a {@link  com.foros.util.csv.CsvWriter CsvWriter} object using an
    ...
    public void write(String content, boolean preserveSpaces) throws IOException {
        ...
        if (!textQualify && userSettings.UseTextQualifier && (content.indexOf(userSettings.TextQualifier) > -1 ||
                content.indexOf(userSettings.Delimiter) > -1 || (!useCustomRecordDelimiter && (content
                .indexOf(Letters.LF) > -1 || content
                .indexOf(Letters.CR) > -1)) || (useCustomRecordDelimiter && content.contains(userSettings.RecordDelimiter)) || (firstColumn && content.length() > 0 && content
    ...
    public void setAppendLineEndingChar(boolean appendLineEndingChar) {
        this.appendLineEndingChar = appendLineEndingChar;
    }
}