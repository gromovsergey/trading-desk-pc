...
public class IBANValidator {
    /**
     * Referene:- http://soastation.blogspot.com/2007/08/java-iban-check-digit-validation.html
    ...
    public static boolean isCheckDigitValid(String iban) {
        ...
        if (StringUtil.isPropertyEmpty(trimmedIban) || trimmedIban.length() < 4) {
            return false;
        }

        int validIBANLength = getValidIBANLength(trimmedIban);

        if (validIBANLength < 4) {
            return false;
        }

        if (trimmedIban.length() != validIBANLength) {
            return false;
        }

        BigInteger numericIBAN = getNumericIBAN(trimmedIban, false);
    ...
    public static int getValidIBANLength(String countryCode) {
        ...
        String length;
        try {
            length = ResourceBundle.getBundle(IBANValidator.class.getSimpleName()).getString("length." + code);
