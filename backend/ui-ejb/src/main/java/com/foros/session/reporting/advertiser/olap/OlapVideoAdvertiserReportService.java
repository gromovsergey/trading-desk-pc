...
@LocalBean
@Stateless(name = "OlapVideoAdvertiserReportService")
@Interceptors({RestrictionInterceptor.class, ValidationInterceptor.class})
public class OlapVideoAdvertiserReportService extends OlapAdvertiserReportServiceBase {
    ...
    protected OlapAdvertiserReportDescription getDescription(OlapAdvertiserReportParameters params) {
        return params.getReportType().getVideoDescription();
    }
    ...
    @Override
    @Restrict(restriction = "Report.VideoAdvertiser.run", parameters = "#parameters")
    ...
    private class Report extends OlapAdvertiserReportServiceBase.Report {
        ...
        public ReportType getReportType() {
            return ReportType.VIDEO_ADVERTISING;
        }
        ...
        protected OlapQuery buildQuery() {
            OlapQuery result = super.buildQuery();
            OlapIdentifier videoSize = parameters.getReportType().resolveIdentifier(OlapDetailLevel.Filter.Size);
            if (videoSize != null) {
                result.filter(videoSize, "[Video (VAST)]");
            }
