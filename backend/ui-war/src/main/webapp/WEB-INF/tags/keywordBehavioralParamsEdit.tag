...
<ui:section titleKey="${bpTitleKey}" tipKey="channel.params.keyword.tooltip" errors="${bpErrorsPath}">

    <ui:fieldGroup>
        <c:forTokens var="bpType" items="${bpTypes}" delims=",">
            <s:set var="currentParamPath" value="#attr.bpPropertyName + '(' + quote(#attr.bpType) + ')'"/>
            <s:set var="currentParam" value="#root[#attr.bpPropertyName].{^ #this.triggerType.toString() == #attr.bpType}[0]"/>

            <tr id="bpRow[${bpType}]" class="bpRow">
                <td>
                    <s:set var="bpEnabled" value="true"/>
                <script type="text/javascript">
                    $(function() {
                ...
                <c:choose>
                    <c:when test="${bpType==URL_TRIGGER_TYPE}">
                        <c:set var="labelText"><fmt:message key="channel.params.urls"/></c:set>
                    ...
                    <c:when test="${bpType==SEARCH_KW_TRIGGER_TYPE}">
                        <c:set var="labelText"><fmt:message key="channel.params.searchKeywords"/></c:set>
                    ...
                    <c:when test="${bpType==PAGE_KW_TRIGGER_TYPE}">
                        <c:set var="labelText"><fmt:message key="channel.params.pageKeywords"/></c:set>
                    ...
                    <c:otherwise>
                        <c:set var="labelText">Unknown type ['<c:out value="${bpType}"/>']</c:set>
                    </c:otherwise>
                </c:choose>
                ...
                <label class="withInput">
                    <input id="bpEnabled[${bpType}]" class="triggerType" type="hidden" value="${bpType}"/>
                </label>
                </td>
                <td class="field">
