...
<#if parameters.method?exists>
 <#assign formMethod = parameters.method?lower_case?html/>
<#else>
...
>
<#if formMethod == "post">
    <#if !Request._principal.anonymous>
     <input type="hidden" name="PWSToken" value="${Session.PWSToken}"/>
    </#if>
</#if>
