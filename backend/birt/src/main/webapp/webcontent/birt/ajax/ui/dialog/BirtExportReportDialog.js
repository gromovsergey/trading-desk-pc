...
BirtExportReportDialog.prototype = Object.extend( new AbstractBaseDialog( ),
{
	...
	__exportAction : function( )
	{
		...
		{	
            ...
            else if( $( 'exportPageAll' ).checked )
            {
                action = action + "&" + Constants.PARAM_PAGERANGE + "=";
            }
    ...
    __enableXlsExport : function( )
	{
		var format = $( 'exportFormat' ).value.toLowerCase( );
		if( this.__isXlsLayout( format ) && !this.__isXlsExportAllowed())
		{
			this.__setDisabled( 'exportReportDialogokButton', true );
            $('xlsExportWarning').style.display = "block";
		}
		else
		{
			this.__setDisabled( 'exportReportDialogokButton', false );
            $('xlsExportWarning').style.display = "none";
		}
	},

    __isXlsLayout : function( format )
	{
		if( !format )
			return false;

		return format == Constants.FORMAT_XLS;
	},
    ...
    __isXlsExportAllowed : function()
    {
        var roughTotal = birtUtility.getRoughTotalElementNumber();
        return roughTotal < Constants.EXPORT_XLS_THRESHOLD;
    },
