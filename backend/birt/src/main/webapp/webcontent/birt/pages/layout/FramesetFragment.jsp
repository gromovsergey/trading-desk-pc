...
<jsp:useBean id="reportSession" type="com.foros.model.report.birt.BirtReportSession" scope="request" />

<%
	String baseUrl;
    String baseHref;

    String baseUrlParam = request.getParameter("baseurl");
    if (baseUrlParam != null) {
        baseUrl = baseUrlParam;
        baseHref = baseUrl;
    } else {
        // base href can be defined in config file for deployment.
        baseHref = request.getScheme( ) + "://" + request.getServerName( ) + ":" + request.getServerPort( );
...
<HTML>
	<HEAD>
		...
		<script type="text/javascript">			
            ...
            var cancelTaskURL = "<%= (String)request.getAttribute("CancelTaskURL")%>";
            var exportURL = "<%= (String)request.getAttribute("ExportURL")%>";
            var extractURL = "<%= (String)request.getAttribute("ExtractURL")%>";
		</script>
				...
				<TD COLSPAN='2'>
					<TABLE BORDER=0 CELLSPACING="0" CELLPADDING="1px" WIDTH="100%">
						<TR>
							<TD WIDTH="3px"/>
							<TD>
								<B><%= attributeBean.getReportTitle( ) %></B>  <%= reportSession.getReport().getName() %>
                            </TD>
							<TD ALIGN='right'>
							</TD>
							<TD WIDTH="3px"/>
