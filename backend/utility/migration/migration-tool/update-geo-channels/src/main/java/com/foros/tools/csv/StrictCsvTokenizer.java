...
public class StrictCsvTokenizer implements Closeable {
                ...
                case QUOTED:
                    if (c == '\n') {
                        if (!lineReader.isEndOfFile()) {
                            ...
                            handleUnexpectedQuotedFieldEnd();
                        }
                        col = 0;
                        continue;
                    } else if (c == quote) {
    ...
    protected void handleUnexpectedCharacter(char c) {
        throwException("errors.csv.unexpectedCharacter: " + c);
    }

    protected void handleUnexpectedQuotedFieldEnd() {
        throwException("errors.csv.unexpectedEndOfField");
    }

    protected void handleEmptyLine() {
        throwException("errors.csv.emptyLine");
    }

    protected void throwException(String msg) {
        String loneColMsg = "errors.csv: line = " + getRawLineNumber() + ", column = " + getRawColumnNumber();
