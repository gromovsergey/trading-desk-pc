...
repo_get_packages() {
  ...
  case $repo in
    local)
      ...
      ;;
    test)
      pattern=`repo_get_search_pattern "/u01/ks/repos/moscow-test/ui" $unknown_packages`
      ;;
    test6)
      pattern=`repo_get_search_pattern "/u01/ks/repos/el6/RPMS.test" $unknown_packages`
      ;;
    stage)
      pattern=`repo_get_search_pattern "/u01/ks/repos/moscow-stage/ui" $unknown_packages`
      ;;
    eme*)
      pattern=`repo_get_search_pattern "/u01/ks/repos/moscow-emergency/ui" $unknown_packages`
      ;;
    devel):
    ...
    any)
      for repo in "local" "test" "stage" "eme" "devel"; do
        local output=`repo_get_packages -r $repo $unknown_packages`
        echo "$output"
        [ -z "$output" ] || return 0
      done
      return 1
  ...
  [ "$?" != "0" ] && exit 1
  [ -z "$output" ] && { echo ": $FUNCNAME: no package found" >&2; return 1; }
...
repo_is_package_in_repo() {
  ...
  local output=`repo_get_packages -r $repo $packages`
  [ -z "$output" ] && return 1
  return 0
}
...
repo_get_repo_by_colo() {
  ...
  case $colo in
    ...
    production-ru-central) echo "eme";;
    moscow-test-ui-at) echo test;;
