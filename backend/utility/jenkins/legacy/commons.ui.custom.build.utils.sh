...
ui_mount_distributed_fs() {
  ...
  doc sudo -Hiu uiuser -i mkdir -p /u01/foros/ui/var/sync
  doc sudo -Hiu uiuser -i mkdir -p /u01/foros/ui/var/www
...
ui_update_dir() {
  ...
  docl sudo -Hiu uiuser "rsync -e \"ssh -i /home/uiuser/.ssh/$ssh_key_file -o 'BatchMode yes'\" -avz $src_host:$dir/* $local_dir"

  echo ": $FUNCNAME: Copying $local_dir to $dst_host:$dir"
  docl sudo -Hiu uiuser "rsync -e \"ssh -i /home/uiuser/.ssh/uikey-moscow-dev-ui -o 'BatchMode yes'\" -avz \"$local_dir/\"* $dst_host:$dir"
...
ui_remove_old_packages() {
  ...
  if [ "$branch" != "none" ]; then
    local host
    for host in $hosts; do
      local packages=`product_get_package_list "ui" -c moscow-dev-ui*`
      uninstall_packages $host $packages
    done
  elif [ "$config_branch" != "none" ]; then
    local host
    for host in $hosts; do
      local packages=`product_get_package_list "ui" -c moscow-dev-ui* -C`
      uninstall_packages $host $packages
    done
  fi
}
...
ui_update() {
  ...
  for host in $hosts; do
    ...
    #if echo $notBuild | grep -q unixcommons; then
    #  docl sudo -Hiu uiuser "scp -q -i '/home/uiuser/.ssh/uikey-moscow-dev-ui' -o 'BatchMode yes' $WORKING_DIR/unixcommons/* $host:/opt/foros/ui/lib"
    #fi

    ssh -o BatchMode=yes $host sudo chown uiuser:uiuser -R /opt/foros/ui/lib/autodeploy
  done
  return 0
}
...
ui_update_ear() {
  ...
  for host in $hosts; do
    ...
    doc sudo -Hiu uiuser "scp -q -i '/home/uiuser/.ssh/uikey-moscow-dev-ui' -o 'BatchMode yes' $filename $host:/opt/foros/ui/lib/autodeploy"
  done
  return 0
}
