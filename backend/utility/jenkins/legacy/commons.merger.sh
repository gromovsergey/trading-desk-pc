...
merger_start() {
  ...
  echo "
[general]
spool-directory = $WORKING_DIR/merger_cache
...
" > $WORKING_DIR/merger.conf
    local params="$WORKING_DIR/merger.conf"
...
merger_stop() {
  ...
  sudo -Hiu uiuser -i kill `cat $WORKING_DIR/merger.pid`
...
merger_func_test() {
  ...
  execute_remote_ex `whoami` stat-dev0 "*.py *.sh" $svn_path $db <<-"EOF"
    ...
    doc "sudo -Hiu uiuser python -u $WORKING_DIR/perf_test.py -- $PG_DATABASE $RESULT_DB $MERGER_FOLDER $MERGER_FAILURE_FOLDER $MERGER_LOG $ROW_COUNT $FILES_COUNT $TIMEOUT"
