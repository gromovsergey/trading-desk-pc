...
update_zenforos_plugin() {
  ...
  # doc zenforos_erase $monitoring_host $colo_name $template
  zenforos_erase $monitoring_host $colo_name $template
  install_packages "$monitoring_host" -r local $packages
  doc zenforos_install $monitoring_host $colo_name $template
  return 0
}
...
test_snmp_walk() {
  ...
  for host in $HOSTS; do
    ...
    docr snmpwalk -v2c -c public -M+/opt/zenoss/lib/foros/$COLO_NAME -mALL $host foros
  done
