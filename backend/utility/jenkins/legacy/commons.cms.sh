...
create_plugin_colocation_xml() {
  ...
  export CMS_HOST=$HOST
  export CMS_HOST_NO=${CMS_HOST##oix-dev}
  ...
  case $PLUGIN in
    "FOROS-UI")
      ...
      envsubst < $CHECKOUT_FOLDER/configs/moscow-dev-colocation.xml > $COLO_XML_FILE || exit 1
      return $?
      ;;
    "FOROS-PGDB")
    ...
    "AdClient")
      ...
      docl svn_export_folder "$SVN_ROOT/$BRANCH/cms-plugin"
      COLO_XML_FILE="$WORKING_DIR/moscow-dev-ui-$HOST-$PLUGIN-`basename $BRANCH`-colocation.xml"
      envsubst < $CHECKOUT_FOLDER/configs/moscow-dev-creatives-colocation.xml > $COLO_XML_FILE || exit 1
      return $?
      ;;
    "FOROS-DSTRADM")
      local SVN_ROOT="svn+ssh://svn/home/svnroot/oix/datastore-adm";
      local TEMPLATE_GENERATOR="dev-datastore-adm-config-generator.sh";
      local TEMPLATE_FILENAME="${HOST}.xml"
      ;;
...
create_config_rpms() {
  ...
  case $PLUGIN in
    ...
    "FOROS-DSTR")
      local SVN_ROOT="svn+ssh://svn/home/svnroot/oix/datastore";
      local CHECKOUT_ADDITIONAL="$SVN_ROOT/$BRANCH/oozie#oozie $SVN_ROOT/$BRANCH/patches#patches $SVN_ROOT/$BRANCH/utility#utility"
      ;;
    "FOROS-DSTRADM")
      local SVN_ROOT="svn+ssh://svn/home/svnroot/oix/datastore-adm";
      local CHECKOUT_ADDITIONAL="$SVN_ROOT/$BRANCH/utility/monitoring/monitoring#utility/monitoring/monitoring"
      ;;
    "AdClient")
  ...
  execute_remote_ex `whoami` cms-test "*.sh *.xml" "$VERSION" "$SVN_ROOT/$BRANCH/$PLUGIN_FOLDER" "$COLO_XML_FILE" "$PLUGIN" "$CHECKOUT_ADDITIONAL" <<-"EOF"
    ...
    if [ "$PLUGIN" = "AdClient" ]; then
      echo ": create_config_rpms: fixing $PLUGIN_FOLDER/data/build/AdClient.spec"
      sed 's|Requires: foros-config-ui.*$||g' -i $PLUGIN_FOLDER/data/build/AdClient.spec
    fi
    ...
    if [ "$PLUGIN" = "AdClient" ]; then
      echo ": create_config_rpms: fixing $PLUGIN_FOLDER/*.xml"
      sed "s/version=\\"[0-9]\\+\\.[0-9]\\+\\.[0-9]\\+\\"/version=\\"$VERSION\\"/" -i $PLUGIN_FOLDER/*.xml
    fi

    sed -i "s/@CMS_CURRENT_VERSION@/$VERSION/g" $COLO_XML_FILE
