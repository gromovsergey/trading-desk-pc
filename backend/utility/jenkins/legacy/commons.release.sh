...
CMS_BASE_URL="https://cms-test.ocslab.com:8181"
...
checkout_project() {
  ...
  if [ "$LDAP_USER" = "oix.project.coordinator" ] ; then
    auth="--username $LDAP_USER --password $LDAP_PASSWORD --non-interactive"
    protocol="http://svn.ocslab.com"
  fi
...
### Downloads cms-plugin descriptor file from CMS and returns
###  APP_NAME = FOROS-UI-3.5.0.13
###  APP_VERSION = 3.5.0.13
get_app_version() {
  local app=$1
  local colo=$2
...
### Returns CMS_RELEASE = cms20150915092716.noarch
...
fix_ui_jdbc_test_properties() {
  ...
  pushd $folder
    sed -i -e "s|_${old_database_number}|_${database_number}|g" test.properties
    svn diff
    svn commit $auth -m "ui-ejb/src/test/resources/test.properties updated" || return 1
  popd
...
create_foros_dstr_branch() {
  ...
  echo "Branching Datastore $branch_version"
  local auth=
  local protocol="svn+ssh://svn/home"
  [ "$LDAP_USER" = "oix.project.coordinator" ] && { auth="--username $LDAP_USER --password $LDAP_PASSWORD --non-interactive" ; protocol="http://svn.ocslab.com" ; }
  svn copy $auth $protocol/svnroot/oix/datastore/trunk $protocol/svnroot/oix/datastore/branches/$branch_version -m "Datastore branch" || return 1
  return 0
}
...
create_foros_dstr_tags() {
  ...
  echo "Tagging Datastore $new_version"
  local auth=
  local protocol="svn+ssh://svn/home"
  [ "$LDAP_USER" = "oix.project.coordinator" ] && { auth="--username $LDAP_USER --password $LDAP_PASSWORD --non-interactive" ; protocol="http://svn.ocslab.com" ; }
  svn copy $auth $protocol/svnroot/oix/datastore/branches/$branch_version $protocol/svnroot/oix/datastore/tags/$new_version -m "Datastore tag" || return 1
  return 0
}
...
print_variables() {
  for name in BAMBOO_SUBTASK COLO LDAP_USER \
              ...
              UI_CMS_RELEASE PGDB_CMS_RELEASE PGADM_CMS_RELEASE; do
...
setup_hosts() {
  ...
  case $colo in
    ...
    moscow-stage-central)
      ...
      ;;
    production-ru-central)
      readonly "UI_HOSTS=ru77prooix00"
      readonly "PG_HOSTS=ru77propostdb00"
      readonly "AD_HOSTS=ru77proadbe00"
      readonly "BI_HOSTS=ru77probi00"
      ;;
    moscow-emergency-central)
      readonly "UI_HOSTS=eoix"
      readonly "PG_HOSTS=epostdb00"
...
get_centos_version() {
  ...
  if [ "$colo" = "moscow-test-central" ]; then
    if [ "$svnpath" != "oix/biadm" -a "$svnpath" != "oix/bi" ]; then
      echo "el7"
    else
      echo "el6"
    fi
...
build_project() {
  ...
  [[ "$colo" = "production-ru-central" ]] && repo="eme"

  local package_name="`echo $svnpath | tr '/' '-'`"
  local package="${package_name}-${version}"
  if [ "$svnpath" = "oix/bi" ]; then
    package_name="$package_name-mondrian"
    package="${package_name}*-${version}"
  fi
  if [ "$svnpath" = "oix/datastore" ]; then
    package_name="$package_name-hdfs-load"
    package="${package_name}-${version}"
  fi

  local centos_version=`get_centos_version $colo $svnpath`

  if ! repo_is_package_in_repo -r $repo "${package}*${centos_version}*.rpm" ; then
    if ! find_product_packages $package_name $version $centos_version; then
      echo
      local options=""
      if [ "$centos_version" = "el7" ]; then
        options="$options -m 3.5.el7"
      fi
      if [ "$svnpath" = "oix/bi" ]; then
        options="$options -D \"'%skipAnalysisDatasources true'\"" # do not install analysis datasources
      fi
      ...
      doc copy_packages_to_repo $repo $centos_version $PRODUCT_PACKAGES
    fi
  else
