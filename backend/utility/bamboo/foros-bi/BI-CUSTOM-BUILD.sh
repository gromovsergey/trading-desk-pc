...
BI_HOST="${bamboo__01_Host}" # pentaho-test
...
BI_DB_NAME="${bamboo__04_DB_Name}" # ui_dev_X | nb_copyX | auto | skip

get_svn_path $BI_SVNPATH
BI_SVNPATH=$SVN_PATH

[ -z "$BI_PGDB_HOST" ] && BI_PGDB_HOST="skip"
[ -z "$BI_HOST" ] && BI_HOST="pentaho-test"
...
case $BAMBOO_SUBTASK in
  ...
  UPDATEBIUI)
    ...
    bi_update_ui $PENTAHO_CONNECTION `abt_get_custom_build_version foros/bi` $BI_HOST
    ;;
  UPDATEBIPGDB)
    bi_update_pgdb $BI_PGDB_HOST `abt_get_custom_build_version foros/bi`
    ;;
  START)
