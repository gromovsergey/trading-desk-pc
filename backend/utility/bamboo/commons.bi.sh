...
update_saiku_schema() {
  ...
  [ -z "$dbname" ] && { echo "Undefined dbname" ; exit 1 ; }

  get_pentaho_connection $dbname

  echo ": $FUNCNAME:"
  execute_remote $host $host $PENTAHO_CONNECTION <<-"EOF"
    WORKING_DIR=`pwd`
    . $WORKING_DIR/commons.sh

    HOST=$1
    PENTAHO_CONNECTION=$2

    SCHEMA="[ForosUi $PENTAHO_CONNECTION].[ForosUi $PENTAHO_CONNECTION].[ForosUi $PENTAHO_CONNECTION]"

    echo ": update_saiku_schema: new schema is $SCHEMA"
    sudo -u uiuser sed -i -e "s|foros.saiku.api.defaultSchema\\" value=\\"\\(.*\\)\\"|foros.saiku.api.defaultSchema\\" value=\\"$SCHEMA\\"|g" \\
      /opt/foros/ui/lib/domains/domain1/config/domain.$HOST.xml &> $WORKING_DIR/saiku.log
    if [ "$?" != "0" ]; then
      cat $WORKING_DIR/saiku.log
      exit 1
    fi
EOF
  [ "$?" != "0" ] && exit 1
...
bi_custom_build() {
  ...
  if [ "${target%%[0-9]}" = "el" ]; then
    abt_setup_custom_build_mock_config "foros/bi" $target
    centos_release="$target"
  else
    abt_setup_custom_build_mock_config_with_host "foros/bi" $target
    centos_release="$CENTOS_RELEASE"
  fi
  abt_custom_build_with_cache "foros/bi" "foros-bi-pgdb" \
    -A "foros/bi foros/bi/$svnpath" \
    -A "foros/db foros/db/trunk" \
    -F "fake/centos:$centos_release" \
    -D "%skipAnalysisDatasources true" \
    -m "$ABT_MOCK_CONFIG"
  abt_save_custom_build_version "foros/bi" "$ABT_VERSION"
}
...
bi_update_pgdb() {
  ...
  doc install_packages_from_repo "$host" "foros-bi-pgdb" "$version"
}
...
bi_update_ui() {
  ...
  [ "$connection" = "skip" ] && { echo ": $FUNCNAME: skipped"; return 0; }
  [ -z "$version" ] && { echo ": $FUNCNAME: undefined version"; exit 1; }
  [ -z "$pentaho_host" ] && pentaho_host="pentaho-test"

  echo ": $FUNCNAME: installing foros-bi-mondrian-$connection-$version to $pentaho_host"

  doc install_packages_from_repo "$pentaho_host" "foros-bi-mondrian-$connection" "$version"
}
