...
biadm_custom_build() {
  ...
  if [ "${target%%[0-9]}" = "el" ]; then
    abt_setup_custom_build_mock_config "foros/biadm" $target
    centos_release="$target"
  else
    abt_setup_custom_build_mock_config_with_host "foros/biadm" $target
    centos_release="$CENTOS_RELEASE"
  fi
  abt_custom_build_with_cache "foros/biadm" "foros-biadm" \
    -A "foros/biadm foros/biadm/$svnpath" \
    -F "fake/centos:$centos_release" \
    -m "$ABT_MOCK_CONFIG"
  abt_save_custom_build_version "foros/biadm" "$ABT_VERSION"
}
...
biadm_update() {
  ...
  doc install_packages_from_repo "$host" "foros-biadm" "$version"
  doc install_packages_from_repo "$host" "foros-config-biadm-$colo" "$version"
