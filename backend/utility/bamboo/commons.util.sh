...
wget_to_store() {
  ...
  mkdir -p "$STORE_PATH" || exit 1
  wget "$http_url" -O "$STORE_PATH/$output_file_name" || exit 1
  if [ "$do_unzip" = "true" ] ; then
    pushd "$STORE_PATH"
    unzip "$output_file_name" || exit 1
    popd
  fi
...
get_centos_release() {
  ...
  CENTOS_RELEASE=`ssh -o BatchMode=yes $host -- rpm -q --qf '%{release}\n' coreutils |\
                  sed -e 's|^.*\(el[0-9]\+\).*$|\1|'`
  [ -z $CENTOS_RELEASE ] && { echo ": $FUNCNAME: $host : could not get centos release"; exit 1; }
}
...
get_required_java_version() {
  ...
  [ "$?" != "0" ] && { echo "$old_version"; return 0; }
  [ -z "$new_version" ] && { echo "$old_version"; return 0; }
  echo "$new_version"
  return 0
}
