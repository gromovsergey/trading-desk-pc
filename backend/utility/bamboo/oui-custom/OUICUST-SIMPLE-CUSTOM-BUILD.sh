...
OUI_REPLICATION="${bamboo__5_Replication}" # yes or no
OUI_REDEPLOY_EAR="${bamboo__6_RedeployEAROnly}" # yes or no
OUI_BI_BRANCH="${bamboo__7_BISvnPath}" # auto or trunk, major(3.0.0), minor(3.1.0.4), branch
...
[ -z "$OUI_REPLICATION" ] && OUI_REPLICATION="no"
...
trunk)
  ...
  [ "$OUI_USE_SELENIUM" = "yes" ] && PG_DBNAME="nb_copy" && ORA_DBNAME="NB_COPY"
  ;;
test)
  DBN=11
  [ "$OUI_USE_SELENIUM" = "yes" ] && DBN=${HOST##oix-dev}
...
export bamboo__06_OracleDB="${ORA_DBNAME}${DBN}"
...
export bamboo__12_Replication="$OUI_REPLICATION"
export bamboo__14_JspWikiBranch="none"
