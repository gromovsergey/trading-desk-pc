...
abt_build_trunk() {
  ...
  local buildboxStorageDir="/tmp/OUI-COMMON/repo/nb/${project##foros/}"
...
abt_setup_custom_build_mock_config() {
  ...
  local mock_config="${project##foros/}_${name}_${centos_release}_${BAMBOO_MOCK_NO:-any}"
...
abt_copy_rpms_to_cache() {
  ...
  [ ! -d "$src" ] && { echo ": $FUNCNAME: src is not directory"; exit 1; }
  [ ! -d "$dst" ] && { echo ": $FUNCNAME: dst is not directory"; exit 1; }
...
# args:
#  project, e.g.: foros/ui
#  version, e.g.: 5000.1.1.1
#  other, e.g.: -A 'foros/ui foros/ui/trunk' - wile be passed to abt
# returns:
...
abt_custom_build() {
  ...
  local storageDir="/tmp/OUI-COMMON/repo/custom/${project##foros/}"
  local tempStorageDir=`mktemp -d /tmp/${project##foros/}-custom-build.XXXXXXXXX`
...
# args:
#  project, e.g.: foros/ui
#  expected_package, e.g.: foros-ui - this name will be checked in cache
#  other, e.g.: -A 'foros/ui foros/ui/trunk' - wile be passed to abt
# returns:
...
abt_custom_build_with_cache() {
  ...
  local branches=`abt_get_branches "$@"`
  local version=""
  local buildboxStorageDir="/tmp/OUI-COMMON/repo/custom/${project##foros/}"
...
abt_get_custom_build_version_file() {
  ...
  echo "/tmp/OUI-COMMON/${project##foros/}-custom-build-${BAMBOO_MOCK_NO:-any}.version"
}
